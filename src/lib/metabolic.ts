...
export function calculateEnergyTarget(input: { weightKg: number; heightCm: number; birthDate: string; metabolicSex: MetabolicSex; activityLevel: string; goal?: string }) {
  ...
  const goalAdjustment = normalizedGoal.includes("perder") || normalizedGoal.includes("emagrecer") ? -300 : normalizedGoal.includes("ganhar") || normalizedGoal.includes("massa") ? 250 : 0;
  const targetKcal = Math.max(1200, Math.round((bmr * factor + goalAdjustment) / 50) * 50);
  return { age, bmr: Math.round(bmr), activityFactor: factor, goalAdjustment, targetKcal, formula: "Mifflin–St Jeor" };
}