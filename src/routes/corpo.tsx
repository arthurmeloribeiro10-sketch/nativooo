...
import { AlertTriangle, Footprints, HeartPulse, LocateFixed, MapPin, Moon, Search, Smartphone, Sun } from "lucide-react";
import { toast } from "sonner";

import { AppShell, PageTitle } from "@/components/nativo/AppShell";
import { Button } from "@/components/ui/button";
import { useAuth } from "@/lib/auth-context";
import { lastDays, today, useHealthData, useProfile, useSaveSleep, useSaveSteps, useSleepWeek, useStepsWeek, weekdayLabel } from "@/lib/nativo-queries";

export const Route = createFileRoute("/corpo")({
  head: () => ({ meta: [
    { title: "Atividade ao ar livre, passos e sono — APOLO" },
    { name: "description", content: "Consulte o índice UV por horário, registre passos e acompanhe seu sono no APOLO." },
...
function CorpoPage() {
  ...
  const profile = useProfile(userId); const stepsWeek = useStepsWeek(userId); const sleepWeek = useSleepWeek(userId); const health = useHealthData(userId);
  ...
  const latestByType = (type: string) => health.data?.samples.find((sample) => sample.metric_type === type);
  const glucose = latestByType("blood_glucose"); const systolic = latestByType("blood_pressure_systolic"); const diastolic = latestByType("blood_pressure_diastolic"); const heartRate = latestByType("heart_rate");
  ...
  return <AppShell>
    ...
    <section className={`surface rise border p-5 ${guidance?.style ?? "border-border"}`}>
      ...
      {!place ? <div className="mt-4 rounded-lg border border-border bg-background/60 p-4">
        ...
        <p className="mt-1 text-xs text-muted-foreground">Sem localização, o APOLO não cria recomendações locais.</p>
    ...
    <section className="surface mt-5 p-5"><div className="flex items-start justify-between gap-4"><div><h2 className="flex items-center gap-2 text-lg"><Smartphone className="size-5 text-leaf"/>Apple Saúde</h2><p className="mt-1 text-xs text-muted-foreground">Dados autorizados no iPhone, incluindo medições de aparelhos compatíveis.</p></div><span className={`rounded-full px-3 py-1 text-xs font-medium ${health.data?.connection?.status === "connected" ? "bg-success/15 text-foreground" : "bg-secondary text-muted-foreground"}`}>{health.data?.connection?.status === "connected" ? "Conectado" : "Não conectado"}</span></div>{health.isLoading ? <p className="mt-4 text-sm text-muted-foreground">Verificando conexão…</p> : health.data?.connection?.status === "connected" ? <><p className="mt-3 text-xs text-muted-foreground">Última sincronização {health.data.connection.last_synced_at ? new Date(health.data.connection.last_synced_at).toLocaleString("pt-BR") : "ainda não informada"}{health.data.connection.device_name ? ` · ${health.data.connection.device_name}` : ""}</p><div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4"><HealthMetric label="Frequência" sample={heartRate}/><HealthMetric label="Glicemia" sample={glucose}/><HealthMetric label="Pressão sistólica" sample={systolic}/><HealthMetric label="Pressão diastólica" sample={diastolic}/></div><p className="mt-3 text-xs text-muted-foreground">Glicemia e pressão servem como contexto e não alteram seu score. Valores preocupantes precisam de avaliação profissional.</p></> : <div className="mt-4 rounded-lg border border-border bg-background/60 p-4"><p className="flex items-center gap-2 text-sm font-medium"><HeartPulse className="size-4 text-terracotta"/>Conexão pronta para o aplicativo do iPhone</p><p className="mt-2 text-xs leading-relaxed text-muted-foreground">O site não pode abrir o Apple Saúde diretamente. A sincronização automática exige o aplicativo companheiro APOLO para iPhone, com sua autorização. O Apple Watch fornece passos, sono e frequência cardíaca; glicemia e pressão numéricas só aparecem quando um medidor ou sensor compatível envia esses dados ao Apple Saúde.</p><Button className="mt-3" variant="outline" disabled>Aplicativo iPhone necessário</Button></div>}</section>

    <div className="mt-5 grid gap-5 lg:grid-cols-2">
      <section className="surface p-5"><h2 className="flex items-center gap-2 text-lg"><Footprints className="size-5 text-leaf"/>Passos de hoje</h2><div className="mt-4 flex items-end justify-between"><p className="font-display text-4xl font-semibold">{stepsTodayRow ? stepsTodayRow.steps.toLocaleString("pt-BR") : "—"}</p><p className="text-xs text-muted-foreground">meta pessoal {goal.toLocaleString("pt-BR")}</p></div><p className="mt-1 text-xs text-muted-foreground">{stepsTodayRow ? stepsTodayRow.source === "apple_health" ? "Importado do Apple Saúde." : "Valor informado por você." : "Ainda sem registro hoje."}</p><div className="mt-4 flex gap-2"><input type="number" min={0} value={stepInput} onChange={(e) => setStepInput(e.target.value)} placeholder="Passos de hoje" className="min-w-0 flex-1 rounded-lg border border-input bg-background px-3 py-2 text-sm"/><Button disabled={saveSteps.isPending} onClick={() => { const value = Number(stepInput); if (!Number.isFinite(value) || value < 0) return; saveSteps.mutate(Math.round(value), { onSuccess: () => { setStepInput(""); toast.success("Passos salvos."); }, onError: () => toast.error("Não foi possível salvar. Tente novamente.") }); }}>Salvar</Button></div><div className="mt-5 flex h-20 items-end gap-2">{weekDays.map((day) => { const value = (stepsWeek.data ?? []).find((d) => d.day === day)?.steps; return <div key={day} className="flex flex-1 flex-col items-center gap-1"><div className="w-full rounded-t bg-leaf/70" style={{ height: value === undefined ? 2 : `${Math.max((value / maxSteps) * 55, 4)}px` }}/><span className="text-[10px] text-muted-foreground">{weekdayLabel(day)}</span></div>; })}</div></section>
...
function HealthMetric({ label, sample }: { label: string; sample: { value: number; unit: string; measured_at: string; source_name: string } | undefined }) {
  return <div className="rounded-lg border border-border bg-background/60 p-3"><p className="text-[11px] text-muted-foreground">{label}</p><p className="mt-1 text-sm font-semibold">{sample ? `${sample.value.toLocaleString("pt-BR")} ${sample.unit}` : "Sem dado"}</p>{sample ? <p className="mt-1 text-[10px] text-muted-foreground">{sample.source_name} · {new Date(sample.measured_at).toLocaleString("pt-BR")}</p> : null}</div>;
}
