...
export const Route = createFileRoute("/comunidade")({
  head: () => ({
    meta: [
      { title: "Comunidade e ranking — Apollo" },
      {
        name: "description",
        content:
      ...
      { property: "og:title", content: "Comunidade e ranking — Apollo" },
      {
  ...
  return (
    <AppShell>
        ...
        <Button
          ...
          aria-label={
            notifications.unreadCount > 0
              ? `${notifications.unreadCount} notificações não lidas`
              : "Notificações"
          }
                ...
                <li key={item.id}>
                  <button
                    ...
                    onClick={() => {
                      ...
                      document
                        .getElementById(`post-${item.post_id}`)
                        ?.scrollIntoView({ behavior: "smooth", block: "start" });
                    }}
                  >
                    <span
                      className={`mt-1.5 size-2 shrink-0 rounded-full ${item.read_at ? "bg-border" : "bg-terracotta"}`}
                    />
                    <span className="min-w-0 flex-1">
                      <span className="block text-sm">
                        ...
                        {item.kind === "reply"
                          ...
                          : " compartilhou uma nova mensagem."}
                      </span>
                      {item.preview ? (
      ...
      <section className="mb-5 flex items-center justify-between">
        <div>
          <h2 className="text-xl">Publicações recentes</h2>
          <p className="text-xs text-muted-foreground">Experiências reais da comunidade.</p>
        </div>
        <button
          onClick={() => setComposerOpen((v) => !v)}
          className="flex min-h-11 items-center gap-2 rounded-lg bg-primary px-4 text-sm font-medium text-primary-foreground"
        >
          {composerOpen ? <ChevronDown className="size-4" /> : <Plus className="size-4" />}
          {composerOpen ? "Fechar" : "Compartilhar"}
        </button>
      </section>
      {composerOpen ? (
      ...
      <section className="mt-5 space-y-3">
        {posts.isLoading ? (
          ...
          <div className="surface p-5">
            <p className="text-sm">Não foi possível carregar as publicações.</p>
            <button
          ...
          <div className="surface p-5">
            <p className="text-sm font-medium">A comunidade ainda está vazia.</p>
            <p className="mt-1 text-xs text-muted-foreground">
              Compartilhe uma experiência quando quiser inaugurar este espaço.
            </p>
          </div>
        ) : (
          (posts.data ?? []).map((p) => (
            <article key={p.id} id={`post-${p.id}`} className="surface scroll-mt-6 p-5">
              <div className="flex items-center gap-3">
                <div className="flex size-10 items-center justify-center rounded-full bg-secondary font-display text-sm">
                ...
                {p.user_id === userId ? (
                  <button
                    ...
                    className="p-2 text-muted-foreground hover:text-terracotta"
                  >
              ...
              <div className="mt-4 flex items-center gap-5">
                <button
                  ...
                  aria-label={p.reacted ? "Remover apoio" : "Apoiar publicação"}
                >
                  <Heart
                    ...
                    <div key={item.id} className="flex gap-3">
                      ...
                      {item.user_id === userId ? (
                        <button
                          ...
                          onClick={() =>
                            removeReply.mutate(item.id, {
                              onError: () => toast.error("Não consegui apagar a resposta."),
                            })
                          }
      ...
      <section className="surface mt-6 p-5">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Histórico acumulado. Ordem: dias do protocolo; em empate, missões concluídas.
        </p>
        {ranking.isLoading ? (
          <p className="mt-4 text-sm text-muted-foreground">Carregando ranking…</p>
        ) : ranking.isError ? (
          <div className="mt-4">
